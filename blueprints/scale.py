...
def _scale_read_plus(ip, port) -> dict:
    """
    Read all PLUs stored on the scale by sending current POS products (MsgNo 1001)
    and parsing the MsgNo 2001 echo the scale sends back.
    Returns list of PLU dicts: plu_no, name, sales_mode, price_cents, tare, shelf_life.
    """
    import select as _select

    # Build records for all sync_to_scale products
    ...
    for p in products:
        ...
        if err:
            continue
        plu_no = p.product_code
        sales_mode = 0 if p.sold_by_weight else 1
        price = _price_cents(p)
        name = (p.name or '')[:20].upper().replace('"', '""')
        unit_line = 'PER KG' if p.sold_by_weight else 'EACH'
        ...
        for idx in [2,3,5,7,10,11,12,17,18,23,24,28,29,30,31,35,42,46,47,48,
            ...
            f[idx] = '0'
        f[0] = str(plu_no); f[1] = str(plu_no)
        f[9] = str(p.scale_shelf_life or 0)
        f[45] = '1' if p.scale_open_price else '0'
        f[49] = f'"{desc}"'; f[50] = str(sales_mode)
        f[51] = str(price); f[52] = str(price); f[56] = '1'
        f[61] = str(int(float(p.scale_tare or 0)))
        f[67] = '21'; f[69] = '20'; f[70] = str(plu_no)
        records.append((','.join(f) + ',').encode('utf-8'))

    if not records:
        return {'ok': True, 'plus': [], 'sent': 0}

    def recv_exact(sock, n):
        buf = bytearray()
        while len(buf) < n:
            r, _, _ = _select.select([sock], [], [], SCALE_TIMEOUT)
            if not r:
                raise IOError(f'Timeout waiting for {n-len(buf)} bytes')
            chunk = sock.recv(n - len(buf))
            if not chunk:
    ...
    try:
        ...
        struct.pack_into('>H', hdr, 6, len(records))
        sock.sendall(bytes(hdr))
        ...
        for i, rec in enumerate(records):
            ...
            sub[2] = 1 if i == 0 else 0
            struct.pack_into('>H', sub, 6, len(rec))
            sock.sendall(bytes(sub))
            sock.sendall(rec)

            if (i + 1) % BATCH == 0:
                r, _, _ = _select.select([sock], [], [], 1)
                if r:
                    raw = recv_exact(sock, 20)
                    total_acked += struct.unpack_from('>H', raw, 14)[0] or struct.unpack_from('>H', raw, 12)[0]
                    ack = bytearray(8)
                    ack[0:2] = _num2bcd(1001, 4)
                    struct.pack_into('>H', ack, 6, total_acked)
                    sock.sendall(bytes(ack))

        # Drain remaining 1001 acks
        remaining = len(records) - total_acked
        while remaining > 0:
            raw = recv_exact(sock, 20)
            batch_total = struct.unpack_from('>H', raw, 14)[0] or struct.unpack_from('>H', raw, 12)[0]
            total_acked += batch_total
            remaining = len(records) - total_acked
            if remaining > 0:
                ack = bytearray(8)
                ack[0:2] = _num2bcd(1001, 4)
        ...
        # Read MsgNo 2001 echo — scale sends back all its stored PLUs
        rh = recv_exact(sock, 8)
        msg_no_bytes = rh[0:2]
        if msg_no_bytes == bytes([0x20, 0x01]):
            total_size = struct.unpack_from('>I', rh, 4)[0]
            remaining_bytes = total_size - 8
            while remaining_bytes > 8:
                sub = recv_exact(sock, 8)
                remaining_bytes -= 8
                data_size = struct.unpack_from('>H', sub, 6)[0]
                if data_size == 0:
                    break
                rec = recv_exact(sock, data_size)
                remaining_bytes -= data_size
                try:
                    csv = rec.decode('utf-8', errors='replace')
                    fields = csv.split(',')
                    if len(fields) > 51 and fields[0].strip().isdigit():
                        raw_name = fields[49].strip().strip('"') if len(fields) > 49 else ''
                        name = ''.join(c for c in raw_name if c.isprintable() and c not in '\r\n\x01').strip()
                        plus.append({
                            'plu_no':      int(fields[0].strip()),
                            'name':        name[:40],
                            'sales_mode':  int(fields[50]) if fields[50].strip().isdigit() else 0,
                            'price_cents': int(fields[51]) if fields[51].strip().isdigit() else 0,
                            'tare':        int(fields[61]) if len(fields) > 61 and fields[61].strip().isdigit() else 0,
                            'shelf_life':  int(fields[9]) if len(fields) > 9 and fields[9].strip().isdigit() else 0,
                        })
                except Exception:
                    pass
    ...
    return {'ok': True, 'plus': plus, 'sent': len(records)}
...
@bp.route('/api/scale/read-plus', methods=['POST'])
def api_scale_read_plus():
    """
    Read all PLUs currently stored on the scale by sending current products
    and parsing the scale's MsgNo 2001 echo response.
    """
    if not require_role('admin'):
        return jsonify({'error': 'Forbidden'}), 403
    ...
    try:
        result = _scale_read_plus(cfg['ip'], cfg['port'])
    except Exception as e:
        logger.error(f"read_plus error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500

    # Cross-reference with POS products
    plu_map = {p.product_code: p for p in
               Product.query.filter(Product.product_code.isnot(None)).all()}

    enriched = []
    for item in result['plus']:
        p = plu_map.get(item['plu_no'])
        enriched.append({
            **item,
            'price_display': f"R{item['price_cents']/100:.2f}" if item['sales_mode'] == 1
                             else f"R{item['price_cents']/1000:.2f}/kg",
            'in_pos':        p is not None,
            'pos_name':      p.name if p else None,
            'sync_to_scale': p.sync_to_scale if p else False,
            'is_archived':   p.is_archived if p else False,
            'product_id':    p.id if p else None,
        })

    enriched.sort(key=lambda x: x['plu_no'])
    return jsonify({'plus': enriched, 'total': len(enriched), 'sent': result['sent']})
