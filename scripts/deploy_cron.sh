...
echo "[$(date)] Starting scheduled deploy #$SCHEDULE_ID: $DESCRIPTION"

# Run deploy
LOG=$(bash "$DEPLOY_SH" prod 2>&1)
EXIT_CODE=$?

SUCCESS="false"
[ $EXIT_CODE -eq 0 ] && SUCCESS="true"

echo "[$(date)] Deploy #$SCHEDULE_ID finished (exit=$EXIT_CODE)"
...
echo "[$(date)] Deploy #$SCHEDULE_ID complete (success=$SUCCESS)"
