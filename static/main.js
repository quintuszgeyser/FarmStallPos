...
function updateProductTypeSections(type) {
  ...
  if (barcodeInput && barcodeInput.dataset.autoGenerated !== '0') {
    if (autoSoldByWeight) {
      // Weight/volume: find next code in range 1-19999, show scale barcode preview
      // (weight part is 00000 — scale fills it in at print time)
      const usedCodes = new Set(STATE.products
        .filter(p => p.product_code >= 1 && p.product_code <= 19999)
        .map(p => p.product_code));
      let nextCode = 1;
      while (usedCodes.has(nextCode) && nextCode <= 19999) nextCode++;
      const core = `20${String(nextCode).padStart(5,'0')}00000`;
      barcodeInput.value = core + ean13Check(core);
      barcodeInput.placeholder = '';
      barcodeInput.title = 'Preview — scale encodes actual weight at print time';
    } else {
