...
document.getElementById('btn-new-product')?.addEventListener('click', () => {
  ...
  if (bcInput) { bcInput.value = ''; bcInput.dataset.autoGenerated = '1'; }
});

// Clear auto-generated flag when user manually edits barcode
document.getElementById('p-barcode')?.addEventListener('input', function() {
...
function updateProductTypeSections(type) {
  ...
  // Update barcode preview based on product type
  const barcodeInput = el('p-barcode');
  if (barcodeInput) {
    const isNew = !el('p-id')?.value || parseInt(el('p-id')?.value) < 0;
    if (isNew || !barcodeInput.value || barcodeInput.dataset.autoGenerated === '1') {
      if (autoSoldByWeight || (isStockItem && unitType === 'volume')) {
        barcodeInput.value = '';
        barcodeInput.placeholder = 'Auto — scale generates from product code';
        barcodeInput.dataset.autoGenerated = '1';
      } else {
        // Preview deterministic barcode from next available product code
        const lo = type === 'recipe' ? 40000 : isSimple || isStockItem ? 20000 : 40000;
        const hi = lo + 9999;
